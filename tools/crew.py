...
DEFAULT_CREW = {
    ...
    "Glass Cat": dict(
        ...
        listen=("minimal and dry: NO hats — a snap keeps time; the clap "
                ...
                "Neptunes flam); bar 8 stop-starts. Gated reverb sits on "
                "the stamp -- the click/zap hook itself, not the snare "
                "(Chad Hugo's own account of the gate reverb on "
                "'Grindin'''s hook sound, confirmed 2026-09-03 -- "
                "everything else, snare included, stays dry). Stamp is a "
                "weird click/zap on bars 4/8"),
        kit=dict(
        ...
        dust=0.0, vinyl=0, wow=0.0, sidechain=0.2,
        # 2026-09-03: moved from ["snare"] to ["stamp"] and alt dropped.
        # Chad Hugo's own account of the "Grindin'" hook sound -- "we added
        # a gate reverb to that sound" -- is about the CLICK/ZAP hook, not
        # the snare; the era-vs-house tension the old alt file existed to
        # flag is resolved by that primary source, not still open.
        space=("gated", ["stamp"]), alt=None,
        drive=1.4, kick_dist=0.0, mix_sat=0.0,
    ),
...
def build_kit(shots, name, stamp_audio, variant=0, avoid=None, preset=None):
    ...
    A preset's `sub_layer` dict (freq_hz/dur_s/amount/drive), if present,
    reinforces the picked KICK with groove.kick_sub_reinforce before it's
    tiled across the beat's hits — has to happen here, on the raw
    one-shot, not later on the assembled buffer (see that function's
    docstring for why). Off unless a preset carries the field; nothing in
    CREW does yet — 2026-09-03 research pass, unaudited, no DJ turned on."""
