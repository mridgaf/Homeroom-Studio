...
- Context: owner asked to research FnZ for new drum-machine builds. FnZ's own drum identity splits in two: on their Drake/Future/Kid Laroi hits they mostly supply a sample and someone else (Vinylz, Tay Keith) adds the drums; on Denzel Curry's Imperial/TA1300/ZUU/Melt My Eyez they build the beat themselves. Owner picked that second, Denzel/ZUU era; picked the roster name himself (Well Damn, from their producer tag "FNZ, well damn!"); picked "add as #14, replace nobody" — same shape as DJ Light Green.
- Decision/change: new legend, num 41, bpm 165 with a halftime clap. Full sourcing (6 sources: Sound On Sound, Highsnobiety, Complex, Wikipedia, What Went Right With..., tunebat/songbpm) lives in `legends_config.json["Well Damn"]["signature"]["_note"]`. bpm sourced from real FnZ-drummed Denzel tempos (Knotty Head 158, Ricky 169, Shake 88 182, P.A.T. 160, Birdz 156-doubled). Kick/clap/hat/perc tags (808/sub/deep/boom kick, clap backbeat 0.95/displaced 0.05, drive16 hats, cowbell) borrowed from the Miami Bass GENRE build done the same session — legitimate because Highsnobiety names 2 Live Crew as ZUU's direct inspiration, and that genre's tags were already audited against real filenames in this library today. kick_dist 6.0 / mix_sat 3.0 / drive 1.6 pushed to the roster's most distorted, matching "steroidal 808s"/"brash" — magnitudes UNSOURCED and flagged as the first thing to change if the audition sounds wrong. Chord signature (minor/dorian, static/aggressive-vamp progressions) also unsourced — no source discusses FnZ's harmony on this era.
- Written to BOTH files a legend lives in: `legends_config.json` (live) and `tools/legends.py` (LEGENDS_DEFAULT, LEGEND_KICK_BANK — 12 fresh patterns, never borrowed — and LEGEND_TITLES), so a config regen would not silently drop him. `tests/test_crew.py` updated: roster count 13->14, `own_soundbank` set +Well Damn, `NO_STAMP_LANE` +Well Damn (kit carries a stamp entry, no stamp lane — same shape as Razor/DJ Light Green, since STAMP_LANE has been False for everyone since 2026-08-01). Backup: `legends_config.pre-well-damn-2026-09-26.json`.
- Reasoning: this session's cloud sandbox is Linux — it cannot reach the TBOTC 3 drive or run the real macOS venv (pedalboard is a macOS-only wheel), the same wall the classifier-reachability-audit skill exists for. Rather than fabricate a tag audit or a rendered audition, the kit tags were borrowed from a same-day, same-library, already-audited build (the Miami Bass genre) instead of guessed fresh, and everything unverified is named as such in the research note rather than reported as done.
- Verify by: what DID run here — `tools/legends.py` and the config both parse; `load_legends(normalize_preset)` accepts "Well Damn" and returns it fully normalized with no error; `tests/test_crew.py -q` gives 7 failed / 33 passed, the same pedalboard-missing + external-drive-missing split already documented for this sandbox, with no new failure introduced. What did NOT run, and still needs his machine: `tools/legend_newbuild.py --legend "Well Damn" --tags` (real tag audit against live filenames — the numbers above are borrowed, not independently verified for this legend), the midi-validity-gate, and an actual rendered/heard audition. `make_legend_newbuild.py`'s old-vs-new diff does not apply here (there is no "old" for a brand-new legend, same as DJ Light Green) — a plain render + README is the right next step, same as DJ Light Green's "NEW LEGEND" folder.
- Status: open
...
### 2026-09-26 Bass "pulses and loses time with the rhythm" — investigated, NOT fixed
- Context: owner, after hearing recent renders: "the bass line in the beats just kind of pulses and loses time with the rhythm."
- Ruled out: the 2026-09-25 ghost-kick duck bug (already fixed, commit 19a60ea, 13:21 that day) — measured dips in the bass stem's envelope on beats 2935/2936 (both rendered ~23:41 that night, after the fix) are NOT correlated with kick onset times at all; they run continuously through the whole note regardless of where kicks land.
- Confirmed as fact: the chordbass lane now holds ONE continuous note for the length of a WHOLE harmony-chord slot — measured 12.0s straight (2935, bar5-8 at 80bpm) and similar spans in 2936 — via `_fit_length`'s crossfaded tile-repeat (instrument_sampler.py). That is a real behavioral fact, not a guess: the bass does not move with the beat at all once a chord holds; it is one static drone under everything else's normal rhythm.
- Tried and ABANDONED: measured Hilbert-envelope autocorrelation looking for a specific "wobble" defect around 30-70Hz in the bass audio. Found strong (corr 0.7-0.98) periodicity in 47 of 50 of his bass one-shots (scanned the whole BOTC Sorted Instruments/Bass pool) — but a control test on a purely synthetic, mathematically static 87.3Hz sawtooth (real harmonics, zero actual modulation) showed the SAME signature (corr=0.976). That means this measurement can't tell a real audible defect from ordinary harmonic content on any bass tone — it fails the audio-fix-verify "a number that cannot fail is not evidence" test. Not reused; do not resurrect this specific check without fixing that control-failure first (a real low-pass filter on the rectified signal at a much lower cutoff, e.g. <15 Hz, would be the correct redo, plus a broader library of held-note recordings from the actual codebase for comparison).
- What's still open: whether the perceived "pulse" is (a) just the fact that the bass holds one drone note for up to 12s while the rest of the beat stays busy — a real architecture fact, confirmed above — or (b) an actual audible artifact in specific source one-shots when stretched that long, which the abandoned check above could not distinguish from normal harmonic timbre.
- Owner confirmed (2026-09-26): sent him the two isolated beats (2935, 2936) at the exact held-note timestamps; he confirmed "its those" — this IS the pulse he means.
- ROOT CAUSE FOUND, with a correct measurement this time (real one-pole envelope follower, cutoff well below the crossfade rate, validated against a synthetic-tone control that reads flat as it should): `_fit_length`'s 30ms-crossfaded tile-repeat (used to stretch a short one-shot bass sample across a long held note) repeats the source roughly every 1.3-1.4s on his real bass files (measured on UNISON_BASS_Commas). 1.3s doesn't divide into ANY tempo (80bpm beat = 0.75s), so the repeat seam drifts against the grid instead of landing on it — this is the literal, measured mechanism behind "pulses and loses time with the rhythm."
- FIX (implemented, `tools/instrument_sampler.py` + `tools/beat_machine.py`): `_fit_length()` and `voice_note()` take a new optional `snap_sec` (one beat's length in seconds); when given, the repeat spacing rounds down to the largest whole number of beats that fits, so the seam lands ON the beat instead of drifting. Only the bass-line's own `voice_note` call in `_build_chords` passes it (`snap_sec=bar_s / num`) — every other caller (chords, arps, everyone else's existing behavior/tests) is unaffected since the default is None.
- Verified (measured, not heard): re-ran `_fit_length` from the LIVE patched file against the real UNISON_BASS_Commas one-shot at 80 and 82bpm. Before: seam every 1.356s (1.81 beats — off-grid). After: seam every 0.750s at 80bpm and 0.731s at 82bpm — both exactly 1.000 beats, both tempos. The fix does what it was built to do.
- Tests: couldn't run his real `.venv` (macOS-only venv, this session is Linux) so pip-installed the suite's real deps (numpy already there; added soundfile, mido, scipy, rapidfuzz, pedalboard 0.9.25, fastapi/uvicorn/httpx/python-multipart) and ran the actual project tests directly. test_instrument_sampler.py: 38/38 pass. test_beat_machine.py: 76/81 pass. test_freedom.py + test_crew.py + test_harmony.py + test_genres.py + test_evolution.py + test_multisample.py: 191/194 pass. All 8 remaining failures (incl. the bass-relevant `test_no_beat_ever_has_two_basses`) reproduce IDENTICALLY with the fix backed out — verified by toggling the one-line call site, not by re-reading the diff — so they're pre-existing / caused by this sandbox only having the Bass subfolder of Sample Packs mounted, not this change. Did NOT run all 45 test files (diminishing returns once the pattern was clear) and did NOT run this on his actual Mac.
- Status: confirmed the mechanism and the fix mathematically (direct simulation against his real sample file, both tempos) and confirmed no regression in ~319 real project tests run in this sandbox. NOT YET confirmed: his own `.venv/bin/python -m pytest tests/ -q` on the Mac (this sandbox's run is a strong signal, not a substitute), and NOT YET heard — no real render exists yet for him to judge by ear. Give him the one pytest command, then a fresh render of the same Trip Hop batch to listen to.

### 2026-09-26 MIDI chord packs added to every DJ / Legend / genre at 10%
- Context: resumed MIDI chord-source session. Wiring already existed and was heard working 2026-09-19; only 5 presets used it (New Math 33%, Rage Engine 23%, Night Metro 10%, Hitt Kid 10%, DJ Light Green 7%).
- Decision: owner picked "Everyone, small share". The other 38 presets (crew 8, legends 12, genres 18) got `["midi", total/9]` appended to `signature.chord_source` = 10% of beats. The 5 existing shares are unchanged. Additions only (152 lines added, 0 removed), original file layouts kept. Backups: `*.pre-midi-everyone-2026-09-26.json`.
